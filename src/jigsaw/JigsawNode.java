...
//	private static final int dimension = 5; 	// 拼图的维数 5*5
	...
	/**
	 * 设置当前节点的深度
	 * @param nodeDepth - 要设置成的节点深度
	 * @attention 该函数不要随便调用防止节点的深度发生错误 =_=
	 */
	...
	/**
	 * 获取节点的子节点，即到达此节点的上一个节点
	 * @return son - 子节点
	 */
	public JigsawNode getSon() {
		return son;
	}
	
	/**
	 * 设置节点的子节点
	 * @param parent  - 要设置成的子节点
	 */
	public void setSon(JigsawNode son){
		this.son = son;
	}
	...
	/**
	 * 用于双向BFS，从目标节点向某一方向移动当前拼图状态中的空白格
	 * @param direction - 方向标记：0为向上，1为向下，2为向左，3为向右
	 * @return 移动成功返回true，失败返回false
	 */
	public boolean rmove(int direction) {
		switch (direction) {
		case 0:
			return this.rmoveEmptyUp();
		case 1:
			return this.rmoveEmptyDown();
		case 2:
			return this.rmoveEmptyLeft();
		case 3:
			return this.rmoveEmptyRight();
		default:
			return false;
		}
	}
	...
	/**
	 * 用于双向BFS，向上移动当前拼图状态中的空白格
	 * @return 移动成功返回true，失败返回false
	 */
	public boolean rmoveEmptyUp() {
		int emptyPos = this.nodesState[0];
		int emptyValue = this.nodesState[emptyPos];
		if (this.nodesState[0] > dimension
				&& this.nodesState[0] <= dimension * dimension) {
	...
	/**
	 * 用于双向BFS，向下移动当前拼图状态中的空白格
	 * @return 移动成功返回true，失败返回false
	 */
	public boolean rmoveEmptyDown() {
		int emptyPos = this.nodesState[0];
		int emptyValue = this.nodesState[emptyPos];
		if (this.nodesState[0] >= 1
				&& this.nodesState[0] <= dimension * (dimension - 1)) {
	...
	/**
	 * 用于双向BFS，向左移动当前拼图状态中的空白格
	 * @return 移动成功返回true，失败返回false
	 */
	public boolean rmoveEmptyLeft() {
		int emptyPos = this.nodesState[0];
		int emptyValue = this.nodesState[emptyPos];
		if (this.nodesState[0] % dimension != 1) {
	...
	/**
	 * 用于双向BFS，向右移动当前拼图状态中的空白格
	 * @return 移动成功返回true，失败返回false
	 */
	public boolean rmoveEmptyRight() {
		int emptyPos = this.nodesState[0];
		int emptyValue = this.nodesState[emptyPos];
		if (this.nodesState[0] % dimension != 0) {
